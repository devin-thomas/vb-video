...
from capture_common import HARNESS, ROOT, asset_dir, dump, now_utc, prepare_workdir, sha256
    ...
    def finish(self, extra: dict) -> None:
        ...
        dump(evidence / "capture.json", {
            ...
            "terminal": {
                ...
                "startup_script": s.setup_script.replace(str(ROOT) + "\\", ""),
            },
...
# ----- tickets -----------------------------------------------------------------------------------
def term01() -> None:
    work = asset_dir("TERM-01") / "work" / "War"
    if work.exists():
...
def main() -> int:
    ...
    error_file = asset_dir(args.id) / "work" / "capture-error.txt"
    try:
        TICKETS[args.id]()
