...
BACKGROUND = "#111318"
HIGHLIGHT = "#EAB676"
SHARED_VERSION = json.loads((ROOT / "assets" / "shared" / "VERSION.json").read_text(encoding="utf-8"))["version"]
...
def asset_dir(asset_id: str) -> Path:
    return ROOT / ticket(asset_id)["asset_dir"]
...
def prepare_workdir(asset_id: str) -> Path:
    ...
    work = asset_dir(asset_id) / "work" / "War"
    if work.exists():
