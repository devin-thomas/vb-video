...
from capture_common import (HARNESS, ROOT, asset_dir, cell_box, contact_sheet, crop_frame, dump, frame, proof_720, sha256,
...
def qa_text(asset_id: str, spec: dict, tests: list[dict], tc: dict) -> str:
    ...
    lines += ["", "## Manual visual inspection", f"- Full size: {review['full_size']}", f"- 720p: {review['proof_720']}", "",
              "## Reproduction", "```",
              f"python tools/war-harness/tools/capture_assets.py --id {asset_id}",
              f"python tools/war-harness/tools/finish_assets.py --id {asset_id} --stage render",
              f"python tools/war-harness/tools/finish_assets.py --id {asset_id} --stage bundle", "```",
...
def bundle_ops02() -> None:
    ...
    log = ["# OPS-02 command log — run by tools/war-harness/tools/build_harness.py via subprocess (no terminal UI)",
...
## What was built
1. `dotnet new console -lang VB` in the empty `tools/war-harness/War/` → exit 0. Untouched output kept in `tools/war-harness/scaffold/`.
...
3. Derived project change, the only edit (`tools/war-harness/project-diff.txt`): added `<OptionExplicit>On</OptionExplicit>` and `<OptionStrict>On</OptionStrict>` after `{manifest['target_framework']}`, which is unchanged. This derived file is not an uploaded original (R17).
...
## Snapshot and copies
`exports/harness-manifest.json` lists every snapshot file with its hash. Capture workers copy `War/War.vbproj` and `War/Program.vb` into `assets/<family>/<ID>/work/War/`, verify both hashes, and build there, so no two captures share `bin/` or `obj/`. Hash-verified copies were used by: {', '.join(users) or 'none yet'}. TERM-01 intentionally scaffolds its own empty directory instead.

## Capture tooling
`tools/war-harness/tools/` holds the real-terminal driver (conhost + pwsh, WriteConsoleInput, PrintWindow, buffer read-back), per-ticket capture scripts, and the finisher. See `tools/war-harness/README.md`.
"""
...
"""
    write_bundle("OPS-02", provenance_type="support infrastructure: derived local .NET harness",
                 variants=[{"name": "harness", "files": ["exports/harness-manifest.json", "exports/report.md"],
                            "snapshot_root": "../../../tools/war-harness/"}],
                 ...
                 gates={"R17": {"evidence": ["exports/harness-manifest.json", "exports/report.md"],
                                "reason": "Derived isolated project generated with the installed SDK; every property change "
                                          "is in tools/war-harness/project-diff.txt."}},
