...
Run from anywhere: python tools/war-harness/tools/build_harness.py
Writes tools/war-harness/{War,scaffold,project-diff.txt,harness-manifest.json} and raw evidence under
assets/ops/OPS-02/evidence/. Refuses to overwrite an existing harness.
"""
...
EVIDENCE = ROOT / "assets" / "ops" / "OPS-02" / "evidence"
...
def main() -> int:
    ...
    manifest = {
        ...
        "files": [{"path": p.relative_to(HARNESS).as_posix(), "bytes": p.stat().st_size, "sha256": sha256(p)}
                  for p in snapshot],
        "copy_rule": "Copy War/War.vbproj and War/Program.vb into assets/<family>/<ID>/work/War/, verify both hashes, "
