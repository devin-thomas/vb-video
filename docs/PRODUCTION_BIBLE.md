...
`tools/fixtures/war_storyboard.json` is an **authored teaching fixture**, not a real terminal run. Its same-scene identities are unique and every counter state sums to 52. `tools/fixtures/deck_order.json` derives from the literal nested loops. `tools/fixtures/shuffle_storyboard.json` is a small deterministic example, not evidence of randomness or unbiasedness. Never present these as captured program execution.
