> Preserved README of the original planning build pack. Links are updated for this repository; prose paths describe the original pack layout. See [../README.md](../README.md) for the current layout.
...
## Start here
1. Read [AGENTS.md](../AGENTS.md) and the [production bible](PRODUCTION_BIBLE.md).
2. Give your coordinator the kickoff prompt in [DISPATCH_PROMPTS.md](DISPATCH_PROMPTS.md). Begin [OPS-01](tickets/OPS-01.md), [OPS-02](tickets/OPS-02.md), and [OPS-03](tickets/OPS-03.md) in parallel when supported.
3. Assign one [full ticket](TICKET_INDEX.md) per worker. Each worker writes only its own asset folder and state file. Use `manifest.json` or `asset_index.csv` to select work; never substitute their short descriptions for the ticket.
...
- [TICKET_INDEX.md](TICKET_INDEX.md): every work order, grouped for navigation.
- [COVERAGE.md](COVERAGE.md): all 50 original families and all 74 inline script visual cues mapped to specific tickets.
- [OUTPUT_CONTRACT.md](OUTPUT_CONTRACT.md): formats, filenames, metadata, provenance, state, and QA requirements.
