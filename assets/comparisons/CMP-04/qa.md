...
## Reproduction
From the repository root, `python tools/render/build_assets.py --id CMP-04` rebuilds the sources (and resets the ticket's state); `render_assets.py`, `qa_browser.py` and `finish_delivery.py` in tools/render, each with `--id CMP-04`, then render, check and finish it. For OPS-01, use `python tools/render/finish_delivery.py --ops-only`. Read tools/render/REBUILD.md first. Tool versions: the toolchain in delivery.json.
...
- The C# side is proposed from the narrated contrast; use exactly 51 down through 1, not down to 0.
- No compile/run is claimed for illustrative counterparts.

## Revision 2 — 2026-09-16 (Devin's cut note 13)

- Change: left-column header changed from "C#" to "C / C++ / C# / Java" (the decrementing for loop is valid in all four; for C that means C99 or later, which allows the `int i` declaration in the for-initializer).
- Edited files: the asset's own SVG states (`src/scene.svg`, `src/scene-0000.svg`, `src/variant-comparison.svg`, `src/variant-token-focus.svg`) and the inlined copy in `src/index.html` (regenerated as `json.dumps` of `scene-0000.svg`, the same way the original build wrote it). `src/brief.json` still carries the ticket's original copy payload headed "C#"; running `build_assets.py --id CMP-04` would regenerate the revision 1 header, so do not run it without re-applying this change.
- Exact reproduction (Windows, from the repository root, `python` = C:\Python314\python.exe):
  `set CAIROCFFI_DLL_DIRECTORIES=C:\msys64\ucrt64\bin`, then `python tools/render/render_assets.py --id CMP-04`, `python tools/render/qa_browser.py --id CMP-04`, `python tools/render/finish_delivery.py --id CMP-04 --owner "Claude Fable 5.1 worker session (revision 2)"`, `python tools/validate_delivery.py --id CMP-04`. Note that `finish_delivery.py --id` also rewrites `docs/tickets/CMP-04.md` (dropping the Revision 2 section) and `review/browser-summary.json` / `review/production-index.json`; those were restored from git after the run and are not part of this delivery.
...
- Tests run and results: `render_assets.py` (all PNGs 1920 x 1080, evidence/render-tests.json rewritten); `qa_browser.py` (offline, deterministic seek, 4 SVG files, no out-of-canvas text; evidence/browser-tests.json); `validate_delivery.py --id CMP-04` (ok). No motion files exist for this still asset.
