...
- Visual review: Full-resolution exports and 720p proof. Viewed exports/early-history.png, exports/great-divide.png and proofs/poster-720.png. All seven milestones appear across the two views with no overlapping labels. early-history ticks every year (1994 and 1996 unlabeled); great-divide shows the break marks, the divide label and its not-to-scale note. Readable at 720p.
...
## Reproduction
From the repository root, `python tools/render/build_assets.py --id DIA-14` rebuilds the sources (and resets the ticket's state); `render_assets.py`, `qa_browser.py` and `finish_delivery.py` in tools/render, each with `--id DIA-14`, then render, check and finish it. For OPS-01, use `python tools/render/finish_delivery.py --ops-only`. Read tools/render/REBUILD.md first. Tool versions: the toolchain in delivery.json.

## Remaining decisions
Assigned gates: R15, R12. See evidence/claim-checks.json and docs/EDITORIAL_REGISTER.md.
...
- All seven milestones are the script’s draft years (SCRIPT.md:112, 116, 697), recorded with quotes in src/milestones.json. No year is verified yet (R15).
- Feature callouts (VB3 and Jet, VB4 and 32-bit, VB6 as the last classic release, the VB.NET runtime change) are left off until verified.
- early-history spaces 1991–1998 in proportion to time. great-divide is schematic and says so; its break marks do not imply a period without development.
- No “today”, support or lifecycle label is shown (R12).
