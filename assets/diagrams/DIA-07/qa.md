...
- Card art source: the faces are not referenced from `assets/shared/cards/`; they were emitted inline by `grid_svg(close=True)` in tools/render/diagrams.py calling `studio.card()` for the first-rank close-up only when the scenes were authored.
- Replacement: 4 face groups in 1 `src/*.svg` files (186 px) were replaced by the win95-workbench-1.1.0 `card()` output from `tools/render/studio.py` at the same identity, position, size, opacity and accent. Each group’s parameters were recovered from the SVG and proved exact by re-emitting the 1.0.0 group byte for byte before it was replaced; the remainder of every file is untouched. Face-down backs are drawn identically by both versions. The scene table inlined in `src/index.html` was rebuilt from the scene SVGs after checking that the old table matched the old files, and came out byte-identical because only the close-up variant (not a motion state) carries faces; `timeline.json`, `build.json`, text, layout and file names are unchanged. `build_assets.py` was not run (it would reset this ticket and rewrite other assets).
- Exports: `python tools/render/render_assets.py --id DIA-07` (the asset’s `src/build.json` driver) re-rendered poster.png, full-grid.png, first-rank-closeup.png, keyframes/start|middle|end.png, preview.mp4 (18 s, 540 frames), contact-sheet.png and proofs/poster-720.png; it also rewrote `evidence/render-tests.json`.
- Browser check: the offline in-memory Chromium check of `tools/render/qa_browser.py` was re-run through a scratch copy that writes only this asset’s `evidence/browser-tests.json` (the original also rewrites `review/browser-summary.json`, outside this ticket): 56 SVG files, 4542 text boxes, no out-of-canvas text, deterministic seek, no network request.
- Manual visual review with the poster at 1920 × 1080 and 1280 wide plus one extracted frame of each MP4 at both sizes: Only variant-first-rank-closeup.svg draws card faces: the four 2s at 186 px show two pips (top and rotated bottom), red hearts/diamonds and black spades/clubs, and the rank-over-suit index in both corners. The poster, the 53 motion states and the frame extracted from preview.mp4 at 10 s draw the index grid; its S/H/D/C column headers are text glyphs, not card faces, and are unchanged, so preview.mp4 was re-encoded but is visually identical.
- `python tools/validate_delivery.py --id DIA-07` was run after delivery.json was rebuilt; the result is in the completion report. Tool versions for this revision: the toolchain in delivery.json.
...
## Reproduction
`python tools/render/render_assets.py --id DIA-07` from the package root for media assets. For OPS-01, use `python tools/render/finish_delivery.py --ops-only`. Read tools/render/REBUILD.md first. Tool versions: assets/ops/OPS-01/exports/capabilities.json.
...
- Grid cells show the Deck array index, not a shuffled order.
- Rank-major order is S,H,D,C within each rank. Final populated index is 51, followed by Index = 52.
