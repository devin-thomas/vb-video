...
## Reproduction
`python tools/render/render_assets.py --id FACT-02` from the package root for media assets. For OPS-01, use `python tools/render/finish_delivery.py --ops-only`. Read tools/render/REBUILD.md first. Tool versions: assets/ops/OPS-01/exports/capabilities.json.

## Remaining decisions
Assigned gates: R10, R15. See evidence/claim-checks.json and docs/EDITORIAL_REGISTER.md.
...
- Face copy is taken from the supplied ticket. Historical claims, where assigned, remain producer-review gates; no verification is implied by this design.
