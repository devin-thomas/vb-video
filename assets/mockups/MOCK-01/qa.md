...
- Card art source: the faces are not referenced from `assets/shared/cards/`; they were emitted inline by `mock_gui` in tools/render/build_assets.py calling `studio.card()` when the scenes were authored.
- Replacement: 8 face groups in 4 `src/*.svg` files (165 px (157 px backs unchanged)) were replaced by the win95-workbench-1.1.0 `card()` output from `tools/render/studio.py` at the same identity, position, size, opacity and accent. Each group’s parameters were recovered from the SVG and proved exact by re-emitting the 1.0.0 group byte for byte before it was replaced; the remainder of every file is untouched. Face-down backs are drawn identically by both versions. The scene table inlined in `src/index.html` was rebuilt from the edited SVGs after checking that the old table matched the old files; `timeline.json`, `build.json`, text, layout and file names are unchanged. `build_assets.py` was not run (it would reset this ticket and rewrite other assets).
- Exports: `python tools/render/render_assets.py --id MOCK-01` (the asset’s `src/build.json` driver) re-rendered poster.png, concept.png, annotated.png, contact-sheet.png and proofs/poster-720.png (still asset, no MP4); it also rewrote `evidence/render-tests.json`.
- Browser check: the offline in-memory Chromium check of `tools/render/qa_browser.py` was re-run through a scratch copy that writes only this asset’s `evidence/browser-tests.json` (the original also rewrites `review/browser-summary.json`, outside this ticket): 4 SVG files, 87 text boxes, no out-of-canvas text, deterministic seek, no network request.
- Manual visual review with the poster at 1920 × 1080 and 1280 wide plus one extracted frame of each MP4 at both sizes: A♠ and K♥ at 165 px on the felt in poster.png, concept.png and annotated.png: the ace with one large pip, the king with crown, K letter and one heart pip, both with the rank-over-suit index in both corners; the 157 px backs, the 25 / 25 / pot 2 counts, the Win95 chrome and the annotations are unchanged.
- `python tools/validate_delivery.py --id MOCK-01` was run after delivery.json was rebuilt; the result is in the completion report. Tool versions for this revision: the toolchain in delivery.json.
...
## Reproduction
`python tools/render/render_assets.py --id MOCK-01` from the package root for media assets. For OPS-01, use `python tools/render/finish_delivery.py --ops-only`. Read tools/render/REBUILD.md first. Tool versions: assets/ops/OPS-01/exports/capabilities.json.
...
- Conceptual GUI only, not an implemented or captured VB4 application.
- Original vector cards. Counts are 25 + 25 + pot 2 = 52.
