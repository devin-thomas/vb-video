...
def render(id):
 base=ROOT/'assets'/id;data=json.loads((base/'src/build.json').read_text());reports=[]
 ...
 (base/'evidence/render-tests.json').write_text(json.dumps({'renderer':'CairoSVG '+cairosvg.__version__,'png_dimensions':'1920x1080','video_probes':reports,'command':f'python shared/render_assets.py --id {id}'},indent=2))
...
def main():
 ...
 if args.skip_existing:ids=[i for i in ids if not (ROOT/'assets'/i/'evidence/render-tests.json').exists()]
