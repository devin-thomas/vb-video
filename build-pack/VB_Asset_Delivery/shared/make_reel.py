...
with tempfile.TemporaryDirectory(prefix='vb-reel-') as tmp:
 tmp=Path(tmp)
 def shot(item):
  n,(id,start,duration,name)=item;src=ROOT/'assets'/id/'exports'/name;out=tmp/f'{n:02}.mp4'
 ...
 output=ROOT/'PREVIEW_REEL.mp4';subprocess.run(['ffmpeg','-v','error','-y','-f','concat','-safe','0','-i',str(concat),'-c','copy','-movflags','+faststart',str(output)],check=True)
 t=0;edl=[]
 for id,start,dur,name in SHOTS:edl.append({'ticket':id,'source':f'assets/{id}/exports/{name}','source_start':start,'reel_start':t,'duration':dur});t+=dur
