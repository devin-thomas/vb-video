...
def main() -> int:
    ...
    for row in rows:
        ...
        state_path = ROOT / 'assets' / row['id'] / 'state.json'
        try:
    ...
    for i in range(1,8):
        if not (ROOT / f'handoffs/H{i:02d}.md').is_file(): errors.append(f'Missing human handoff H{i:02d}')
    # Derived deck and illustrative fixture integrity, not a test of Program.vb execution.
    deck = json.loads((ROOT / 'fixtures/deck_order.json').read_text())['cards']
    ...
    war = json.loads((ROOT / 'fixtures/war_storyboard.json').read_text())
    ...
    shuffle = json.loads((ROOT / 'fixtures/shuffle_storyboard.json').read_text())
